...
async function dbCall(path, opts={}) {
  ...
  try {
    const r = await fetch(`${SB_URL}/rest/v1/${path}`, {
      ...rest,
      headers: { apikey:SB_KEY, Authorization:`Bearer ${SB_KEY}`, "Content-Type":"application/json", Prefer:"return=representation", ...xh },
    });
  ...
  return(
    <div style={{minHeight:"100vh",background:T.login,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'DM Sans','Segoe UI',sans-serif",padding:20}}>
      <div style={{background:"#fff",border:`1px solid ${T.border}`,borderRadius:20,padding:"40px 36px",width:"100%",maxWidth:400}}>
        ...
        <div style={{fontSize:11,color:T.primary,marginTop:14,textAlign:"center"}}>{cl.owner} · Default PIN: 0000 · Change after first login</div>
      </div>
    </div>
  );
}
...
function MainApp({cl,user,onLogout}){
  ...
  const nav=[{id:"dashboard",label:"Dashboard",icon:"dashboard"},{id:"sales",label:"Sales",icon:"sales"},...(P.exp?[{id:"expenses",label:"Expenses",icon:"expense"}]:[]),{id:"stock",label:"Inventory",icon:"stock"},{id:"credit",label:"Credit Book",icon:"credit"},...(isOwner?[{id:"branches",label:"Branches",icon:"branch"},{id:"users",label:"Users & Access",icon:"users"}]:[])];
  const S={
    ...
    <div style={{display:"flex",height:"100vh",background:T.light,color:T.dark,fontFamily:"'DM Sans','Segoe UI',sans-serif",overflow:"hidden"}}>
      ...
      <main style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        <div style={{flex:1,overflow:"auto",padding:"18px 20px",background:T.light}}>
          ...
          {tab==="credit"&&<CreditPage data={fC} myBranch={myBranch} myBranches={myBranches} P={P} T={T} S={S} Btn={Btn} Del={Del} Tag={Tag} FL={FL} FG={FG} FC={FC} TH={TH} Grid={Grid} Mdl={Mdl} onPayment={doPay} onAdd={doAddCred} onDelete={id=>ask("Delete credit?",()=>doDel("credit",id))} showToast={t2}/>}
          ...
          {tab==="users"&&isOwner&&<UsersPage users={users} branches={branches} T={T} S={S} Btn={Btn} Tag={Tag} FL={FL} FG={FG} FC={FC} TH={TH} Grid={Grid} Mdl={Mdl} onAdd={doAddUser} onToggle={id=>ask("Toggle user?",()=>doToggle(id))} onPin={doPin} showToast={t2}/>}
        </div>
      </main>
...
function StockPage({data,products,myBranch,myBranches,P,T,S,Btn,Tag,FL,FG,FC,TH,Grid,onRestock,onAddProd,showToast}){
  ...
  return<div>
    ...
    {oP&&<FC title="🆕 Add New Product to Catalogue"><FG>
      <FL l="Product Name *"><input style={S.inp} placeholder="e.g. Jollof Rice" value={pf.name} onChange={e=>FP("name",e.target.value)}/></FL>
      <FL l="Unit"><input style={S.inp} placeholder="plate / pack / portion" value={pf.unit} onChange={e=>FP("unit",e.target.value)}/></FL>
...
function CreditPage({data,myBranch,myBranches,P,T,S,Btn,Del,Tag,FL,FG,FC,TH,Grid,Mdl,onPayment,onAdd,onDelete,showToast}){
  ...
  return<div>
    ...
    {modal&&<Mdl onClose={()=>setModal(null)} title={`Record Payment — ${modal.customer}`}>
...
function UsersPage({users,branches,T,S,Btn,Tag,FL,FG,FC,TH,Grid,Mdl,onAdd,onToggle,onPin,showToast}){
  ...
  return<div>
    ...
    {pm&&<Mdl onClose={()=>setPm(null)} title={`Change PIN — ${pm.name}`}>
