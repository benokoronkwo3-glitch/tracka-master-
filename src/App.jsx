...
const CLIENTS = {
  ...
  mr_don: {
    id:"mr_don", name:"Mr Don Clothing",
    address:"Shop GEJ #53/55 Winners, Young Shall Grow Plaza, Main Market, Onitsha",
    state:"Anambra State", owner:"Mr Don Nwoye", phone:"08060488518", logo:"MD",
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 20px",borderBottom:`1px solid ${T.border}`,flexShrink:0,background:"#fff"}}>
          ...
          <div style={{textAlign:"right",fontSize:11,color:"#374151",lineHeight:1.7}}>
            ...
            {isOwner&&<button onClick={()=>{
              ...
              const yearS=sales.filter(s=>new Date(s.date).getFullYear()===now.getFullYear());
              const todayE=expenses.filter(e=>new Date(e.date).toDateString()===now.toDateString());
              const weekE=expenses.filter(e=>{const d=new Date(e.date);const w=new Date(now);w.setDate(now.getDate()-7);return d>=w;});
