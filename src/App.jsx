...
const CLIENTS = {
  ...
  five_star: {
    id:"five_star", name:"Five Star & Food Restaurant",
    address:"Multi-Branch Fast Food · Anambra & Imo States",
    state:"Anambra State", owner:"Dcn Chinedu Akuesodu", phone:"08033422500", logo:"FS",
    ...
    theme:{primary:"#c9320a",dark:"#7f1d1d",light:"#fff7f5",mid:"#ffe4e0",border:"#fecaca",login:"linear-gradient(135deg,#7f1d1d,#c9320a,#f59e0b)",logo:"linear-gradient(135deg,#c9320a,#f59e0b)"},
  },

  hugeman: {
    id:"hugeman", name:"Hugeman Come Ventures Ltd",
    address:"A5/98 God's Line, Marine Modern Market, Onitsha",
    state:"Anambra State", owner:"Dcn Chidiebere Eze", phone:"08037907167", logo:"HC",
    ...
    theme:{primary:"#7c3aed",dark:"#4c1d95",light:"#faf5ff",mid:"#ede9fe",border:"#ddd6fe",login:"linear-gradient(135deg,#4c1d95,#7c3aed,#a78bfa)",logo:"linear-gradient(135deg,#7c3aed,#a78bfa)"},
  },

  trust_divine: {
    id:"trust_divine", name:"Trust Divine Nig Ltd",
    address:"C67 Tobacco Line 5, Main Market, Onitsha",
    state:"Anambra State", owner:"Dcn Paul Orji", phone:"08033081409", logo:"TD",
    ...
    theme:{primary:"#16a34a",dark:"#14532d",light:"#f0fdf4",mid:"#dcfce7",border:"#bbf7d0",login:"linear-gradient(135deg,#14532d,#16a34a,#4ade80)",logo:"linear-gradient(135deg,#16a34a,#4ade80)"},
  },

  cwin_agro: {
    id:"cwin_agro", name:"Cwin Agro Ventures",
    address:"75 Limca Road, Obosi, Onitsha",
    state:"Anambra State", owner:"Godwin Chukwuma", phone:"08060598873", logo:"CA",
    ...
    theme:{primary:"#0369a1",dark:"#0c4a6e",light:"#f0f9ff",mid:"#e0f2fe",border:"#bae6fd",login:"linear-gradient(135deg,#0c4a6e,#0369a1,#38bdf8)",logo:"linear-gradient(135deg,#0369a1,#38bdf8)"},
  },
...
export default function App(){
  // Store client in sessionStorage so URL stays clean after first load
  const getClient=()=>{
    const p=new URLSearchParams(window.location.search);
    const fromUrl=p.get("client");
    if(fromUrl&&CLIENTS[fromUrl]){
      sessionStorage.setItem("tracka_client",fromUrl);
      // Clean URL without removing client access
...
function MainApp({cl,user,onLogout}){
  ...
  const loadAll=useCallback(async()=>{
    ...
    try{
      const[u,p,st,sa,ex,cr]=await Promise.all([
        db.get("users",CID,"order=name.asc"),db.get("products",CID,"order=name.asc"),
        db.get("stock",CID,"order=productName.asc"),db.get("sales",CID,"order=date.desc"),
        db.get("expenses",CID,"order=date.desc"),db.get("credits",CID,"order=due.asc"),
      ]);
      setDbOk(!u.error);
      ...
      if(ex.data?.length){setExpenses(ex.data);const ec=[...new Set(ex.data.map(e=>e.category).filter(Boolean))];setExpCats(v=>[...new Set([...v,...ec])]);}
  ...
  const tS=fS.reduce((s,r)=>s+ +r.amount,0),tE=fE.reduce((s,r)=>s+ +r.amount,0);
  const tC=fC.reduce((s,r)=>s+(+r.amount-+(r.paid||0)),0);
  const lSt=fSt.filter(s=>+s.qty<=+s.reorder),ov=fC.filter(c=>(+c.amount-+(c.paid||0))>0&&c.status==="Overdue");
  const doSale=async d=>{
    ...
    if(error){t2("Sale error: "+(error.message||JSON.stringify(error)),"error");return;}
    ...
    setSales(p=>[row,...p]);t2("Sale saved! Inventory updated ✓");
  };
  const doRestock=async d=>{
    const ex=stock.find(s=>s.productId===d.productId&&s.branch===d.branch);
    if(ex){const q=+ex.qty+ +d.qty;const pt={qty:q,costPrice:d.costPrice||ex.costPrice,sellPrice:d.sellPrice||ex.sellPrice};await db.patch("stock",ex.id,pt);setStock(p=>p.map(s=>s.id===ex.id?{...s,...pt}:s));t2("Restocked ✓ Qty: "+q);}
    else{const row={client_id:CID,...d,id:genId()};await db.post("stock",row);setStock(p=>[...p,row]);t2("Stock added ✓");}
  };
  const doPay=async(id,amt)=>{
    ...
    setCredits(p=>p.map(x=>x.id===id?{...x,paid:np,status:ns}:x));t2("Payment "+fmt(amt)+" recorded ✓");
  };
  const doDel=async(type,id)=>{
    if(type==="sale"){const s=sales.find(x=>x.id===id);if(s){const sr=stock.find(x=>x.productId===s.productId&&x.branch===s.branch);if(sr){const q=+sr.qty+ +s.qty;await db.patch("stock",sr.id,{qty:q});setStock(p=>p.map(x=>x.id===sr.id?{...x,qty:q}:x));}}setSales(p=>p.filter(x=>x.id!==id));}
  ...
  const doAddCred=async d=>{const row={client_id:CID,...d,id:genId(),paid:0,addedBy:user.id};const{error}=await db.post("credits",row);if(error){t2("Error","error");return;}setCredits(p=>[row,...p]);t2("Credit added ✓");};
  const doAddProd=async d=>{const row={client_id:CID,...d,id:genId()};const{error}=await db.post("products",row);if(error){t2("Error","error");return;}setProducts(p=>[...p,row]);t2("Product added ✓");};
  const doAddCat=cat=>{if(expCats.includes(cat)){t2("Already exists","error");return;}setExpCats(p=>[...p,cat]);t2(cat+" added ✓");};
  ...
  const nav=[{id:"dashboard",label:"Dashboard",icon:"dashboard"},{id:"sales",label:"Sales",icon:"sales"},...(P.exp?[{id:"expenses",label:"Expenses",icon:"expense"}]:[]),{id:"stock",label:"Inventory",icon:"stock"},{id:"credit",label:"Credit Book",icon:"credit"},...(isOwner?[{id:"reports",label:"Reports",icon:"report"},{id:"branches",label:"Branches",icon:"branch"},{id:"users",label:"Users & Access",icon:"users"}]:[])];
  const S={
    ...
    <div style={{display:"flex",height:"100vh",background:T.light,color:T.dark,fontFamily:"'DM Sans','Segoe UI',sans-serif",overflow:"hidden"}}>
      <aside style={{width:232,background:"#fff",borderRight:`1px solid ${T.border}`,display:"flex",flexDirection:"column",flexShrink:0}}>
        ...
        <nav style={{flex:1,padding:"10px 7px",display:"flex",flexDirection:"column",gap:2,overflowY:"auto"}}>
          {nav.map(({id,label,icon})=><button key={id} onClick={()=>setTab(id)} style={{display:"flex",alignItems:"center",gap:8,padding:"9px 10px",borderRadius:8,border:"none",background:tab===id?T.mid:"transparent",color:tab===id?T.primary:"#374151",fontSize:13,fontWeight:600,cursor:"pointer",textAlign:"left",width:"100%"}}><Ic name={icon} size={16}/><span style={{flex:1}}>{label}</span>{id==="credit"&&ov.length>0&&<span style={{background:"#ef4444",color:"#fff",fontSize:10,fontWeight:800,borderRadius:99,padding:"1px 6px"}}>{ov.length}</span>}{id==="stock"&&lSt.length>0&&<span style={{background:"#f59e0b",color:"#fff",fontSize:10,fontWeight:800,borderRadius:99,padding:"1px 6px"}}>{lSt.length}</span>}</button>)}
        </nav>
        <div style={{padding:"10px 10px 14px",borderTop:`1px solid ${T.border}`}}>
          ...
          <div style={{fontSize:11,fontWeight:700,marginBottom:8,padding:"4px 8px",background:T.light,borderRadius:6,border:`1px solid ${T.border}`,color:T.primary}}>{user?.role==="owner"?"👑 Owner":user?.role==="manager"?"🏪 Manager":"🧑 Staff"}{!isOwner&&user?.branch&&<span style={{color:"#374151",marginLeft:6}}>· {user.branch}</span>}</div>
      ...
      <main style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 20px",borderBottom:`1px solid ${T.border}`,flexShrink:0,background:"#fff"}}>
          ...
          <div style={{textAlign:"right",fontSize:11,color:"#374151",lineHeight:1.7}}><div style={{fontWeight:700,color:T.primary}}>{cl.owner}</div><div>{cl.phone} · {cl.state}</div></div>
        </div>
        ...
        <div style={{flex:1,overflow:"auto",padding:"18px 20px",background:T.light}}>
          {tab==="dashboard"&&(()=>{
            const kpis=P.fin?[{label:"Total Sales",value:fmt(tS),icon:"sales",color:T.primary,bg:T.mid},{label:"Total Expenses",value:fmt(tE),icon:"expense",color:"#ef4444",bg:"#fef2f2"},{label:"Net Profit",value:fmt(tS-tE),icon:"profit",color:tS-tE>=0?T.primary:"#ef4444",bg:tS-tE>=0?T.mid:"#fef2f2"},{label:"Credit Outstanding",value:fmt(tC),icon:"credit",color:"#f59e0b",bg:"#fef9c3"}]:[{label:"Branch Sales",value:fmt(tS),icon:"sales",color:T.primary,bg:T.mid},{label:"Credit Outstanding",value:fmt(tC),icon:"credit",color:"#f59e0b",bg:"#fef9c3"}];
            return<div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(190px,1fr))",gap:12,marginBottom:16}}>{kpis.map(k=><div key={k.label} style={{background:"#fff",border:`1px solid ${T.border}`,borderRadius:11,padding:"15px 15px 15px 11px",display:"flex",alignItems:"center",gap:12,borderLeft:`4px solid ${k.color}`}}><div style={{width:42,height:42,borderRadius:10,background:k.bg,color:k.color,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><Ic name={k.icon} size={22}/></div><div><div style={{fontSize:12,color:"#374151",marginBottom:3}}>{k.label}</div><div style={{fontSize:22,fontWeight:800,color:k.color}}>{k.value}</div></div></div>)}</div>
              {(lSt.length>0||ov.length>0)&&<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))",gap:12,marginBottom:16}}>{lSt.length>0&&<div style={{border:"1px solid #f59e0b",borderRadius:10,padding:13,background:"#fff"}}><div style={{color:"#f59e0b",fontWeight:700,display:"flex",alignItems:"center",gap:6,marginBottom:8}}><Ic name="alert" size={15}/>{lSt.length} Low Stock</div>{lSt.map(s=><div key={s.id} style={{fontSize:12,color:"#374151",padding:"3px 0"}}>{s.productName} · {s.branch} — {s.qty} {s.unit} left</div>)}</div>}{ov.length>0&&<div style={{border:"1px solid #ef4444",borderRadius:10,padding:13,background:"#fff"}}><div style={{color:"#ef4444",fontWeight:700,display:"flex",alignItems:"center",gap:6,marginBottom:8}}><Ic name="alert" size={15}/>{ov.length} Overdue Credit</div>{ov.map(c=><div key={c.id} style={{fontSize:12,color:"#374151",padding:"3px 0"}}>{c.customer} · {c.branch} — {fmt(+c.amount-+(c.paid||0))}</div>)}</div>}</div>}
              {isOwner&&!ab&&<div style={{marginBottom:18}}><SH>Branch Snapshot</SH><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(190px,1fr))",gap:12}}>{branches.map(b=>{const bS=sales.filter(s=>s.branch===b).reduce((t,r)=>t+ +r.amount,0);const bE=expenses.filter(e=>e.branch===b).reduce((t,r)=>t+ +r.amount,0);return<div key={b} style={{background:"#fff",border:`1px solid ${T.border}`,borderRadius:10,padding:14}}><div style={{fontWeight:700,color:T.primary,marginBottom:10,fontSize:13}}>📍 {b}</div><KV l="Sales" v={fmt(bS)} c={T.primary}/><KV l="Expenses" v={fmt(bE)} c="#ef4444"/><KV l="Profit" v={fmt(bS-bE)} c={bS-bE>=0?T.primary:"#ef4444"}/></div>})}</div></div>}
              <div style={{background:"#fff",border:`1px solid ${T.border}`,borderRadius:11,padding:15}}><SH>Recent Sales {ab?"— "+ab:""}</SH><Grid cols={["Product","Customer","Qty","Unit Price","Total","Pay","Branch","Date"]} rows={fS.slice(0,10).map(s=>[s.productName,s.customer||"Walk-in",s.qty,<span style={{color:"#374151"}}>{fmt(s.unitPrice)}</span>,<strong style={{color:T.primary}}>{fmt(s.amount)}</strong>,<Tag bg={T.mid} tc={T.primary}>{s.payType||"Cash"}</Tag>,<Tag>{s.branch}</Tag>,s.date])}/></div>
            </div>;
          })()}
          {tab==="sales"&&<SalesPage data={fS} stock={fSt} products={products} users={users} user={user} myBranch={myBranch} myBranches={myBranches} isOwner={isOwner} P={P} T={T} S={S} Btn={Btn} Del={Del} Tag={Tag} FL={FL} FG={FG} FC={FC} TH={TH} Grid={Grid} onSale={doSale} onDelete={id=>ask("Delete this sale? Stock will be restored.",()=>doDel("sale",id))}/>}
          ...
          {tab==="reports"&&isOwner&&<ReportsPage sales={sales} expenses={expenses} credits={credits} branches={branches} client={cl} ab={ab} T={T} S={S} Btn={Btn} Tag={Tag} TH={TH} Grid={Grid} KV={KV}/>}
...
function SalesPage({data,stock,products,users,user,myBranch,myBranches,isOwner,P,T,S,Btn,Del,Tag,FL,FG,FC,TH,Grid,onSale,onDelete}){
  ...
  const sub=()=>{
    if(!form.productId){alert("Select a product");return;}if(+form.qty<1){alert("Enter valid qty");return;}
    if(!sel){alert("Product not in stock for this branch");return;}if(+form.qty>+sel.qty){alert("Only "+sel.qty+" "+sel.unit+" left");return;}
...
function StockPage({data,products,myBranch,myBranches,P,T,S,Btn,Tag,FL,FG,FC,TH,Grid,onRestock,onAddProd,showToast}){
  ...
  return<div>
    ...
    {oP&&<FC title="🆕 Add New Product to Catalogue"><FG>
      ...
      <FL l="Unit"><input style={S.inp} placeholder="yards / bags / pcs / plate" value={pf.unit} onChange={e=>FP("unit",e.target.value)}/></FL>
...
function CreditPage({data,myBranch,myBranches,P,T,S,Btn,Del,Tag,FL,FG,FC,TH,Grid,Modal,onPayment,onAdd,onDelete,showToast}){
  ...
  return<div>
    ...
    {modal&&<Modal onClose={()=>setModal(null)} title={`Record Payment — ${modal.customer}`}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:18}}>{[["Total",fmt(modal.amount),"#374151"],["Paid",fmt(modal.paid||0),T.primary],["Balance",fmt(+modal.amount-+(modal.paid||0)),"#f59e0b"]].map(([l,v,c])=><div key={l} style={{background:T.light,borderRadius:8,padding:10,textAlign:"center"}}><div style={{fontSize:11,color:T.primary}}>{l}</div><div style={{fontWeight:800,color:c,fontSize:14}}>{v}</div></div>)}</div>
      ...
      <div style={{display:"flex",gap:10,marginTop:16}}><button style={S.canc} onClick={()=>setModal(null)}>Cancel</button><button style={S.save} onClick={()=>{const a=+amt;if(!a||a<=0){showToast("Enter valid amount","error");return;}if(a>+modal.amount-+(modal.paid||0)){showToast("Exceeds balance","error");return;}onPayment(modal.id,a);setModal(null);setAmt("");}}>✅ Record Payment</button></div>
    </Modal>}
...
function ReportsPage({sales,expenses,credits,branches,client,ab,T,S,Btn,Tag,TH,Grid,KV}){
  const[period,setPeriod]=useState("today");
  const[branch,setBranch]=useState(ab||"all");
  const[showWA,setShowWA]=useState(null);
  const[waData,setWaData]=useState({customer:"",phone:"",amount:"",item:"",balance:""});
  const now=new Date();
  ...
  const prodTotals={};
  fS.forEach(s=>{if(!prodTotals[s.productName])prodTotals[s.productName]={qty:0,amount:0};prodTotals[s.productName].qty+= +s.qty;prodTotals[s.productName].amount+= +s.amount;});
  const topProds=Object.entries(prodTotals).sort((a,b)=>b[1].amount-a[1].amount).slice(0,5);
  const catTotals={};
  fE.forEach(e=>{catTotals[e.category]=(catTotals[e.category]||0)+ +e.amount;});
  const waReceipt=()=>{
    ...
    return`https://wa.me/${waData.phone.replace(/\D/g,"")}?text=${encodeURIComponent(msg)}`;
  };
  const waThankYou=()=>{
  ...
  return<div>
    <TH title="Reports & WhatsApp Tools"/>
    <div style={{display:"flex",gap:8,marginBottom:18,flexWrap:"wrap",alignItems:"center"}}>
      <span style={{fontSize:12,color:"#374151",fontWeight:700}}>Period:</span>
      {[["today","Today"],["week","This Week"],["month","This Month"],["all","All Time"]].map(([v,l])=><button key={v} onClick={()=>setPeriod(v)} style={ps(period===v)}>{l}</button>)}
      <span style={{fontSize:12,color:"#374151",fontWeight:700,marginLeft:8}}>Branch:</span>
    ...
    {Object.keys(catTotals).length>0&&<div style={card}><div style={{fontWeight:800,color:"#ef4444",fontSize:13,marginBottom:12,textTransform:"uppercase",letterSpacing:.5}}>💸 Expenses by Category</div><Grid cols={["Category","Amount"]} rows={Object.entries(catTotals).sort((a,b)=>b[1]-a[1]).map(([c,a])=>[c,<strong style={{color:"#ef4444"}}>{fmt(a)}</strong>])}/></div>}
    {branch==="all"&&branches.length>1&&<div style={card}><div style={{fontWeight:800,color:T.primary,fontSize:13,marginBottom:12,textTransform:"uppercase",letterSpacing:.5}}>📍 Performance by Branch</div><Grid cols={["Branch","Sales","Expenses","Profit"]} rows={branches.map(b=>{const bS=sales.filter(s=>s.branch===b&&inPeriod(s.date)).reduce((t,r)=>t+ +r.amount,0);const bE=expenses.filter(e=>e.branch===b&&inPeriod(e.date)).reduce((t,r)=>t+ +r.amount,0);return[<strong style={{color:T.dark}}>📍 {b}</strong>,<span style={{color:T.primary}}>{fmt(bS)}</span>,<span style={{color:"#ef4444"}}>{fmt(bE)}</span>,<strong style={{color:bS-bE>=0?T.primary:"#ef4444"}}>{fmt(bS-bE)}</strong>];})}/></div>}
    <div style={{...card,border:"1px solid #25d36640"}}>
      ...
      {overdue.length>0&&<div><div style={{fontWeight:800,color:"#ef4444",fontSize:13,marginBottom:12}}>⚠️ Overdue Credit Reminders</div><div style={{display:"flex",flexDirection:"column",gap:8}}>{overdue.map(c=>{const bal=+c.amount-+(c.paid||0);return<div key={c.id} style={{background:"#fff",border:"1px solid #fca5a5",borderRadius:10,padding:"12px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}><div><div style={{fontWeight:700,color:"#7f1d1d"}}>{c.customer}</div><div style={{fontSize:12,color:"#374151"}}>{c.phone} · {c.branch} · Due: {c.due}</div><div style={{fontSize:13,fontWeight:700,color:"#ef4444"}}>Balance: {fmt(bal)}</div></div><a href={waReminder(c)} target="_blank" rel="noreferrer" style={{background:"#25d366",border:"none",borderRadius:8,color:"#fff",padding:"8px 16px",fontWeight:700,fontSize:13,cursor:"pointer",textDecoration:"none",display:"flex",alignItems:"center",gap:6}}><Ic name="wa" size={16}/> Send Reminder</a></div>;})}</div></div>}
      {overdue.length===0&&<div style={{fontSize:13,color:"#16a34a",fontWeight:600,padding:"8px 0"}}>✅ No overdue credits — all clear!</div>}
    </div>
    <div style={card}><div style={{fontWeight:800,color:T.primary,fontSize:13,marginBottom:12,textTransform:"uppercase",letterSpacing:.5}}>📋 Sales Records — {period==="today"?"Today":period==="week"?"This Week":period==="month"?"This Month":"All Time"} {branch!=="all"?`· ${branch}`:""}</div><Grid cols={["Product","Customer","Qty","Unit Price","Total","Pay","Branch","Date"]} rows={fS.map(s=>[s.productName,s.customer||"Walk-in",s.qty,<span style={{color:"#374151"}}>{fmt(s.unitPrice)}</span>,<strong style={{color:T.primary}}>{fmt(s.amount)}</strong>,<Tag bg={T.mid} tc={T.primary}>{s.payType||"Cash"}</Tag>,<Tag>{s.branch}</Tag>,s.date])}/></div>
  </div>;
}
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:16}}>
      {branches.map(b=>{
        ...
        return<div key={b} style={{background:"#fff",border:`1px solid ${T.border}`,borderRadius:11,padding:16}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}><span style={{fontWeight:800,fontSize:14,color:T.primary}}>📍 {b}</span><button onClick={()=>onSwitch(b)} style={S.pay}><Ic name="switch" size={12}/> View</button></div>
...
function UsersPage({users,branches,T,S,Btn,Tag,FL,FG,FC,TH,Grid,Modal,onAdd,onToggle,onPin,showToast}){
  ...
  return<div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(190px,1fr))",gap:12,marginBottom:20}}>{[{role:"owner",title:"👑 Owner",desc:"All branches · Full financials · Delete · Manage users"},{role:"manager",title:"🏪 Manager",desc:"Own branch only · Sales, Expenses, Stock, Credits"},{role:"staff",title:"🧑 Staff",desc:"Own branch only · Sales only"}].map(r=><div key={r.role} style={{background:T.light,border:`1px solid ${RC[r.role]}40`,borderRadius:11,padding:14}}><div style={{fontWeight:800,color:RC[r.role],marginBottom:6,fontSize:13}}>{r.title}</div><div style={{fontSize:11,color:"#374151",lineHeight:1.7}}>{r.desc}</div></div>)}</div>
    ...
    </FG><div style={{marginTop:12,background:T.mid,borderRadius:8,padding:12,fontSize:12,color:T.primary}}>⚠ This user will only see data for <strong>{form.branch}</strong>.</div>
    ...
    {pm&&<Modal onClose={()=>setPm(null)} title={`Change PIN — ${pm.name}`}>
      ...
      <div style={{display:"flex",gap:10,marginTop:16}}><button style={S.canc} onClick={()=>setPm(null)}>Cancel</button><button style={S.save} onClick={()=>{if(!np||np.length<4){showToast("PIN must be 4–8 digits","error");return;}onPin(pm.id,np);setPm(null);setNp("");}}>Update PIN</button></div>
    </Modal>}
